...
def get_certification(takeoff, student):
    ...
    # fifty hours: student[6]
    cert_at_tk = utils.str_to_time(student[6], takeoff)
    if not cert_at_tk is None and cert_at_tk < takeoff:
        return PILOT_50_HOURS

    # license: student[5]
    cert_at_tk = utils.str_to_time(student[5], takeoff)
    if not cert_at_tk is None and cert_at_tk < takeoff:
        return PILOT_CERTIFIED

    # solo: student[4]
    cert_at_tk = utils.str_to_time(student[4], takeoff)
    if not cert_at_tk is None and cert_at_tk < takeoff:
        return PILOT_STUDENT

    # join: student[3]
    cert_at_tk = utils.str_to_time(student[3], takeoff)
    if not cert_at_tk is None and cert_at_tk < takeoff:
        return PILOT_NOVICE

    # flight before student joined school
...
def get_minimums(cert, area, instructed, vfr, daytime, minimums):
    ...
    for row in minimums[1:]:
        ...
        col2 = (row[2] == area) or (row[2] == 'Local' and area !=
                                    'Cross Country') or (row[2] == 'Any')
    ...
    return [get_best_value(matches, 4, False),
            get_best_value(matches, 5, False),
            get_best_value(matches, 6, True),
            get_best_value(matches, 7, True)]
