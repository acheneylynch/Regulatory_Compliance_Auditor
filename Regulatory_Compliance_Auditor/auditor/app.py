...
def discover_violations(directory, output):
    ...
    # generate the weather violations
    w_v_result = violations.list_weather_violations(directory)
    ...
    # 1 violation
    elif len(w_v_result) == 1:
        print(str(len(w_v_result)) + ' violation found.')

    # 2 or more violations
    else:
        print(str(len(w_v_result)) + ' violations found.')

    if output != None:
        # extract column header from lessons.csv file
        lessons = utils.read_csv(os.path.join(directory, 'lessons.csv'))

        # write to file (i.e., output): lessons.csv header, add 'REASON' and violation
        utils.write_csv([lessons[0]+['REASON']] + w_v_result, output)
