...
def bad_winds(winds, maxwind, maxcross):
    ...
    if winds["units"] == 'KT':
        ...
        if len(winds) == 3:
            ...
            if winds["speed"] > maxwind or winds["gusts"] > maxwind or winds["crosswind"] > maxcross:
                return True
        ...
        if len(winds) == 3:
            ...
            if (winds["speed"] * 1.94384) > maxwind or (winds["crosswind"] * 1.94384) > maxcross:
                return True
            ...
            if (winds["speed"] * 1.94384) > maxwind or (winds["gusts"] * 1.94384) > maxwind or (winds["crosswind"] * 1.94384) > maxcross:
                return True
...
def bad_ceiling(ceiling, minimum):
    ...
    if type(ceiling) == list:

        for item in range(len(ceiling)):

            if ceiling[item]['type'] == 'broken':
                val_list.append(ceiling[item]['height'])

            elif ceiling[item]['type'] == 'overcast':
                val_list.append(ceiling[item]['height'])

            elif ceiling[item]['type'] == 'indefinite ceiling':
                val_list.append(ceiling[item]['height'])

        if len(val_list) > 0:

            if min(val_list) < minimum:
                return True
...
def get_weather_report(takeoff, weather):
    ...
    elif not takeoff.isoformat() in weather:

        for item in weather:
            key_to_dt_obj = utils.str_to_time(item)

            if key_to_dt_obj < takeoff:
                return weather[key_to_dt_obj.isoformat()]
...
def get_weather_violation(weather, minimums):
    ...
    try:
        b_v_result = bad_visibility(weather["visibility"], minimums[1])
        b_w_result = bad_winds(weather["wind"], minimums[2], minimums[3])
        b_c_result = bad_ceiling(weather["sky"], minimums[0])

        # no violations
        if not b_v_result and not b_w_result and not b_c_result:
            return ''

        # three violations
        elif b_v_result and b_w_result and b_c_result:
            return 'Weather'

        # two violations: bad visibility; bad winds
        elif b_v_result and b_w_result and not b_c_result:
            return 'Weather'

        # two violations: bad visibility; bad ceiling
        elif b_v_result and not b_w_result and b_c_result:
            return 'Weather'

        # two violations: bad winds; bad ceiling
        elif not b_v_result and b_w_result and b_c_result:
            return 'Weather'

        # one violation: bad visibility
        elif b_v_result and not b_w_result and not b_c_result:
            return 'Visibility'

        # one violation: bad winds
        elif not b_v_result and b_w_result and not b_c_result:
            return 'Winds'

        # one violation: bad ceiling
        elif not b_v_result and not b_w_result and b_c_result:
            return 'Ceiling'
...
def list_weather_violations(directory):
    ...
    for item in lessons[1:]:
        ...
        # generate takeoff time and during day
        takeoff = utils.str_to_time(item[3], daycycle['timezone'])
        during_day = utils.daytime(takeoff, daycycle)

        # generate pilot credentials
        student = utils.get_for_id(item[0], students)
        pilot_cert = pilots.get_certification(takeoff, student)

        # generate pilot minimums
        instructor = item[2]
        filed = item[5]
        area = item[6]
        pilot_mins = pilots.get_minimums(
            pilot_cert, area, instructor, filed == 'VFR', during_day, minimums)

        # generate weather conditions
        weather_report = get_weather_report(takeoff, weather)

        # check for violation
        violation = get_weather_violation(weather_report, pilot_mins)

        # if violation but not an empty string, add to result
        if violation != '':
            result.append(item+[violation])
